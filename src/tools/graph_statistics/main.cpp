...
int main(int argc, char* argv[]) {
    ...
    auto in_degree_future = std::async([&graph]{ return ygraph::GetInDegree(graph); });
    auto out_degree_future = std::async([&graph]{ return ygraph::GetOutDegree(graph); });
    auto distance_from_center_future = std::async([&graph, &args]{
        return GetDistanceFromCenter(graph, args.graph_center);
    });
    PrintStatistics(
